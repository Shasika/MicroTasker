...
function seedDemo(PDO $pdo): void
{
    ...
    $pdo->prepare('INSERT INTO users(name, email, password, created_at) VALUES (?,?,?,?)')->execute(['Demo Admin', 'admin@demo.test', $password, $now]);
    $pdo->prepare('INSERT INTO users(name, email, password, created_at) VALUES (?,?,?,?)')->execute(['Demo Member', 'member@demo.test', $password, $now]);

    $adminId = (int) $pdo->lastInsertId() - 1;
    $memberId = $adminId + 1;

    $pdo->prepare('INSERT INTO teams(name, created_by, created_at) VALUES (?,?,?)')->execute(['Demo Team', $adminId, $now]);
    ...
    $today = date('Y-m-d');
    $addTask->execute([$teamId, 'Client update', 'Send project update to client', $adminId, 'todo', 'high', $today, null, $adminId, $now, $now]);
    $addTask->execute([$teamId, 'Bug triage', 'Review overnight issues', $memberId, 'doing', 'med', $today, null, $adminId, $now, $now]);
    $addTask->execute([$teamId, 'Deploy hotfix', 'Ship fix for login edge case', $adminId, 'done', 'high', $today, $now, $adminId, $now, $now]);

    $addCheckin = $pdo->prepare('INSERT INTO checkins(team_id,user_id,day,yesterday,today,blockers,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?)');
    $addCheckin->execute([$teamId, $adminId, $today, 'Finished QA', 'Prepare sprint notes', 'Need PM approval', $now, $now]);
    $addCheckin->execute([$teamId, $memberId, $today, 'Refactored API tests', 'Implement board filters', '', $now, $now]);
}
...
function requireTeamMember(int $teamId, int $userId): array
{
    ...
    if (!$member) {
        ...
        exit('Not authorized for this team');
    }
